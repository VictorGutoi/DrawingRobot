...
def build_runner(source: str, geometry: RobotGeometry) -> tuple[CommandRunner, str]:
    """Returns (runner, error_message). On parse error, runner is empty."""
    try:
        cmds = parse_script(source, geometry)
...
def run() -> None:
    ...
    initial_runner, _ = build_runner(initial_source, geometry)
    ...
    def rebuild_runner():
        state.runner, err = build_runner(state.program_source, state.geometry)
        if err:
    ...
    def on_console_submit(line: str):
        try:
            new_cmds: list[WheelCommand] = parse_script(line, state.geometry)
        except ScriptError as e:
    ...
    while True:
        ...
        if state.running and not state.runner.done:
            v_left, v_right = state.runner.current_velocities()
            state.pose = step(state.pose, v_left, v_right, state.geometry.width, dt)
        ...
        if state.running:
            state.current_segment().append(pen_world)
