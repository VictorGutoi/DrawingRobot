...
the robot side:
  ...
  * the Pi-side `pi_service` (this repo) handles codes 80..89 as
    "run stored script slot 0..9".

The firmware ignores unknown codes, so the two ranges coexist.

Mode codes are kept identical to the parent package so the same Int8
buttons / voice / remote sources can drive either implementation
unchanged.
...
class ModePublisher:
    ...
    def __init__(self, topic: str = DEFAULT_TOPIC,
                 node_name: str = "drawingrobot_failsafe_mode_publisher",
                 verbose: bool = True):
        try:
            import rclpy
...
class ModeListener:
    ...
    several codes arrive between drains only the latest one is seen. Caller
    must drive the executor; use `spin_once(timeout_s)` from the main loop.
    """

    DEFAULT_TOPIC = ModePublisher.DEFAULT_TOPIC

    def __init__(self, topic: str = DEFAULT_TOPIC,
                 node_name: str = "drawingrobot_failsafe_mode_listener",
                 verbose: bool = True):
        try:
            import rclpy
            from std_msgs.msg import Int8
