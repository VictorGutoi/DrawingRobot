...
`drawingrobot.limits.Limits(max_linear_cm_s, max_angular_rad_s)` enforces ceilings on the body's `(v, ω)`. Two clamp entry points:

- **`Limits.apply_to_command(cmd, wheelbase)`** — used by `parse_script`. Decomposes a `WheelCommand` into body `(v, ω)`, scales both by `r = min(1, v_max/|v|, ω_max/|ω|)` when either exceeds its ceiling, and stretches `duration` by `1/r` so the total distance and angle are preserved. Curvature `R = v/ω` is preserved; the body follows the same arc, just slower.
- **`Limits.clamp_vw(v, ω)`** — stateless, used at ROS publish time as a defensive net. Same scaling ratio, no duration stretch.

**Limits apply end-to-end: planner + simulator + publish.** The planner consumes `Limits` so the emitted `WheelCommand` stream already satisfies the ceilings; the simulator preview is therefore an honest preview of what the real robot will execute, not just of planner intent. `clamp_vw` at publish stays in place as a safety net for cases where commands escape the planner (e.g., width slider changed mid-run so a parse-time-correct `WheelCommand` now produces an out-of-spec `ω`).

For `trace` specifically, the planner uses **per-step time-scaling**: each tick computes the un-clamped `(v, ω)` demand from feedback linearisation, scales both by `r`, and advances the path parameter by `r · speed · TRACE_DT` instead of the full nominal step. The pen still tracks the polyline exactly — body just dwells longer near sharp corners so peak `|ω|` stays legal. With `pen_body = (p_x, p_y)` and pen-tangential speed `speed`, the natural per-corner `|ω|` peak is `~speed/p_x`; when this exceeds `ω_max`, the corner gets stretched by factor `speed/(p_x · ω_max)`.

**Long-plan warning.** `parse_script` sums the durations of the emitted commands; if the total exceeds `DURATION_WARNING_S` (60 s), it calls `on_warning(...)` with a one-line warning naming the total and current limits. Default `on_warning=print`; `sim.py` overrides this to route the warning into the on-screen console.

For non-trace commands (`forward`, `turn`, `arc`, `circle`, plus the rotate/forward sub-commands inside `_plan_goto` and `_plan_line_to`), `apply_to_command` does the clamping at parse time. With `Limits = NO_LIMITS` (default for tests and docs figures), all the clamping reduces to a no-op and behavior matches the legacy unclamped planner.

`drawingrobot.ros_publisher.RosPublisher(topic, node_name)` publishes `(v, ω)` as `geometry_msgs/Twist` (linear.x = v converted to m/s, angular.z = ω in rad/s). The module imports `rclpy` and `geometry_msgs` lazily inside `__init__` so the rest of the codebase stays usable on machines without ROS2; calling `RosPublisher()` without a ROS2 install raises a `RuntimeError` with a clear message. The publisher is created in `sim.run()` only when `--ros` is passed; teardown on QUIT publishes a final `(0, 0)` Twist (with `force=True`) before `close()`.

**Publish throttle.** Non-zero `(v, ω)` goes out every tick. Idle `(0, 0)` is throttled to a heartbeat rate (`heartbeat_s`, default 1.0 s) so the consumer still sees periodic life signals without flooding the topic when nothing is moving. `force=True` bypasses the throttle (used for the final stop Twist at shutdown). The decision lives in `RosPublisher._should_publish`, kept as a pure static method so it's unit-testable without `rclpy` installed. Both `sim.run` and `run_headless` go through this same path — `sim.run` previously inlined a raw `rclpy` publisher; that duplication is gone.
